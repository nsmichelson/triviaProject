...
def create_app(test_config=None):
    ...
    def getQuestions():
        ...
        return jsonify({
            ...
            "current_category":
            list(set([question['category'] for question in current_questions]))
            })
    ...
    def quizMaker():
        ...
        if quiz_category['id']:
            ...
            Question.query.filter(Question.category ==
                quiz_category['id']).order_by(Question.id).all()
        else:
    ...
    def addQuestion():
        ...
        if searchTerm:
            searchterm = request.get_json().get('searchTerm')
            selection =
            Question.query.order_by(Question.id).filter(Question.question.ilike('%{}%'.format(searchterm)))
            current_questions = paginate_questions(request, selection)
